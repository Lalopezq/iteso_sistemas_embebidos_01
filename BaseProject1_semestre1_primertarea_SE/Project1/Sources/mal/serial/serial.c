...
/* Reception array */
UINT8  u8SCI1_RxData[ SCI1_RX_MAX_SIZE ];
/* Length of received frame */
UINT8  u8SCI1_RxLength;
/* Pointer to current location on Reception array */
UINT8 * pu8SCI1_Receive_Data_ptr;
/* Pointer to actual location being read from the input buffer */
UINT8 * pu8SCI1_Read_Receive_Data_ptr;
/* Transmision array */
UINT8  u8SCI1_TxData[ SCI1_TX_MAX_SIZE ];
/* Length of received frame */
UINT8  u8SCI1_TxLength;
/* Pointer to actual location being read from the output buffer */
UINT8 * pu8SCI1_Read_Transmit_Data_ptr;
#endif
...
void vfnSCI_OFF( UINT8  u8SCIPort )
{
    switch( u8SCIPort )
    {
        #ifdef SERIAL_COMM_CHANNEL0
        case ( SCI_PORT0 ):
            #ifdef __S12_CORE                               /*Disable Reception and Transmission              */ 
            SCI0CR2 &= 0xF3u;
            #endif
            #ifdef __S08_CORE
            #endif
        ...
        case ( SCI_PORT0 ):
            ...
            if( u8SCI0_Comm_Status == SCI_COMM_ON )
            {
                for( u8Index = 0u;
                     ...
                     u8Index++ )
                {
                    u8SCI0_TxData[ u8Index ] = *pu8TxDataBuffer;
                    pu8TxDataBuffer++;
                }
        ...
        case ( SCI_PORT1 ):
            ...
            if( u8SCI1_Comm_Status == SCI_COMM_ON )
            {
                for( u8Index = 0; 
                     ...
                     u8Index++ )
                {
                    u8SCI1_TxData[ u8Index ] = *pu8TxDataBuffer;
                    pu8TxDataBuffer++;
                }
...
void vfnSCI_ClearRx( UINT8  u8SCIPort )
{
 ...
 /* Reset auxiliary variables for input buffer management */
        #ifdef SERIAL_COMM_CHANNEL0
        case ( SCI_PORT0 ):
            pu8SCI0_Receive_Data_ptr      = &u8SCI0_RxData[ 0 ];
 ...
 /* Reset auxiliary variables for input buffer management */
            #ifdef SERIAL_COMM_CHANNEL1
        case ( SCI_PORT1 ):
            pu8SCI1_Receive_Data_ptr      = &u8SCI1_RxData[ 0 ];
...
void vfnSCI_ClearTx( UINT8  u8SCIPort )
{
 ...
 /* Reset auxiliary variables for output buffer management */
        ...
        case ( SCI_PORT0 ):
            ...
            SCI0CR2_TIE                    = CLEAR;
            pu8SCI0_Read_Transmit_Data_ptr = &u8SCI0_TxData[ 0 ];
            u8SCI0_TxLength                = 0u;
 ...
 /* Reset auxiliary variables for output buffer management */
        ...
        case ( SCI_PORT1 ):
            /* inhibit interrupt-driven Tx scheme */
            SCI1CR2_TIE                    = CLEAR;
            pu8SCI1_Read_Transmit_Data_ptr = &u8SCI1_TxData[ 0 ];
            u8SCI1_TxLength                = 0u;
            if( u8SCI0_Comm_Status == SCI_COMM_TX_BUSY )
            {
                u8SCI1_Comm_Status = SCI_COMM_ON;
